...
//    NSString *path = [bundle pathForResource:[NSString stringWithFormat:@"%@@3x",name] ofType:@"png" inDirectory:@"DHNativePhoto.bundle"];
...
-(UIButton *)cancleButton{
    if (_cancleButton == nil) {
        ...
        _cancleButton.frame = CGRectMake(kScreenWidth-40, 15, 30, 30);
        [_cancleButton setImage:SmartImageNamed(@"back_write") forState:(UIControlStateNormal)];
...
-(UIButton *)reCamButton{
    if (_reCamButton == nil) {
        ...
        [_reCamButton setTitle:@"Una pesada" forState:UIControlStateNormal];
        _reCamButton.titleLabel.font = [UIFont systemFontOfSize:13];
...
-(UIButton *)selectButton{
    if (_selectButton == nil) {
        ...
        [_selectButton setTitle:@"determinar" forState:UIControlStateNormal];
...
- (UIImageView *)kuangImgView {
    if (_kuangImgView == nil) {
        _kuangImgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0,  258, 438)];
        _kuangImgView.center = self.view.center;
        _kuangImgView.layer.cornerRadius = 8;
...
- (UILabel *)tishiLabel {
    ...
    if (_tishiLabel == nil) {
        _tishiLabel = [[UILabel alloc] initWithFrame:CGRectMake(-(kScreenWidth/2 -60),kScreenHeight/2 - 10,kScreenHeight * 0.5,40)];
        
        _tishiLabel.text = @"Por favor, tome una foto del frente de su CNIC";
...
-(AVCaptureDevice *)device{
    if (_device == nil) {
        ...
        //_device = [AVCaptureDevice defaultDeviceWithMediaType:AVCaptureDevicePositionFront];
        //_device.position = AVCaptureDevicePositionFront;
...
//        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"请打开相机权限" message:@"设置-隐私-相机" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:@"取消", nil];
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"Por favor active el permiso de la cámara." message:@"Configuración - Privacidad - Cámara" delegate:self cancelButtonTitle:@"Seguro" otherButtonTitles:@"Cancelar", nil];
        alertView.tag = 100;
        [alertView show];
        return NO;
    }
